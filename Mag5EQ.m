...
for ii=1:length(zonelist)
    ...
    for i=1:length(y)
        ...
        endweek{ii} = find(squaredate{ii} <= m5date(i));
        
        begweek{ii} = find(squaredate{ii} > (m5date(i) - 7));
        
        numbf = length(intersect(endweek{ii}, begweek{ii}));
        
        % Find the number of earthquakes a week after
        
        date4{ii} = find(squaredate{ii} >= m5date(i));
        
        date5{ii} = find(squaredate{ii} < (m5date(i) + 7));
