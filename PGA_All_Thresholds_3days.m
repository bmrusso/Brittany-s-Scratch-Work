...
squaredate(10) = [ ];
PGA_total =  [PGA_total(1:7), PGA_total(7), PGA_total(8:12), PGA_total(7),...
    PGA_total(13)];
PGA_total(10) = [ ];

% Setting the Threshold
threshold = [0.0024, 0.0010, 0.00045];
...
for zones=1:length(year_vector)
    ...
    % Removing the data bad data from the earthquake data
        ...
        PGA_total{zones}(remove_vector{zones}) = [ ];
...
for thres=1:length(threshold)
    ...
    for ii=1:length(year_cell)
        ...
        index = find(PGA_total{ii} >= threshold(thres));
